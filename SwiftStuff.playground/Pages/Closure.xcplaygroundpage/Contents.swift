...
sorted = arr.sort( { $0 < $1 })
/*: 
 Voidaan antaa lopulta pelkkä operaattori, koska Int luokan operaattoreilla `<` ja `>`
...
/*:
 ### map, reduce, jne...
 */
var inverted = strings.map({ String($0.characters.reverse()) })
var reduced = strings.reduce("", combine: { $0 + String($1[0]) }) // $1[0] kutsu ei standardi APIssa
print(inverted)
print(reduced)
